...
$app->delete('/user/delete', function (Request $request, Response $response) {
    ...
    $useridToDeleteTokens = $data->userid;

    if ($useridFromToken != $useridToDeleteTokens) {
        return $response->withStatus(403)->write(json_encode(array("status" => "fail", "data" => array("title" => "Unauthorized action"))));
    }
    ...
    try {
        ...
        $stmt = $conn->prepare($sql);
        $stmt->bindParam(':userid', $useridToDeleteTokens);
        $stmt->execute();

        // Optionally mark the original token as used if needed
        markTokenAsUsed($token);
...
$app->delete('/author/delete', function (Request $request, Response $response) {
    ...
    $authoridToDeleteTokens = $data->authorid;
    ...
    try {
        ...
        // Delete all tokens related to the author
        $sql = "DELETE FROM tokens WHERE userid IN (SELECT userid FROM books WHERE authorid = :authorid)";
        $stmt = $conn->prepare($sql);
        $stmt->bindParam(':authorid', $authoridToDeleteTokens);
        $stmt->execute();
...
$app->delete('/book/delete', function (Request $request, Response $response) {
    ...
    $bookidToDeleteTokens = $data->bookid;
    ...
    try {
        ...
        // Delete all tokens related to the book
        $sql = "DELETE FROM tokens WHERE userid IN (SELECT userid FROM books WHERE bookid = :bookid)";
        $stmt = $conn->prepare($sql);
        $stmt->bindParam(':bookid', $bookidToDeleteTokens);
        $stmt->execute();
...
$app->delete('/book_author/delete', function (Request $request, Response $response) {
    ...
    $collectionidToDeleteTokens = $data->collectionid;
    ...
    try {
        ...
        // Delete all tokens related to the collection
        $sql = "DELETE FROM tokens WHERE userid IN (SELECT userid FROM book_author WHERE collectionid = :collectionid)";
        $stmt = $conn->prepare($sql);
        $stmt->bindParam(':collectionid', $collectionidToDeleteTokens);
        $stmt->execute();
